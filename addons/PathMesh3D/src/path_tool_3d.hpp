...
protected:
    ...
    _FORCE_INLINE_ real_t _sample_offset_modifiers_at(real_t p_offset_ratio, real_t baked_l) const { 
        real_t out = 0.0; 
        for (PathModifier3D *modifier : modifiers) { 
            out += modifier->sample_offset_modifier_at(p_offset_ratio, baked_l); 
        } 
        return out;
    }
